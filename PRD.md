...
- 文风选择：润色先提供默认文风选项，支持正式、公文、汇报、精炼、有感染力等文风，并允许用户输入自定义文风提示词。
- 润色生成：对文本做语言润色、病句修正、表达风格调整和可读性优化，不额外引入理论素材。
- 润色标注：采用类似修订模式的逐字差异标注，支持查看原文与润色后内容。
- 升华入口：用户可从首页或写作结果页触发文章升华。
- 经典理论关联：升华结合用户选择的「学习专栏」中的国家领导人讲话、「金句用典」和灰度入口「先贤经典」发掘与文章主题、观点、场景相关的素材。
...
- 结果交付：润色、升华和排版结果均支持在线预览、下载，并提供可编辑入口；用户点击后进入编辑页面继续修改。
- 模板上传：支持用户上传 100MB 以下的 Word 或 PPT 文件作为格式来源。
- 格式提取：完整复刻上传文件中的页眉页脚、目录、编号、多级标题、封面、PPT 母版，以及字体、字号、行距、段距、页边距、主题色、版式占位符等格式。
...
| polish_custom_prompt | text | 否 | 用户自定义润色文风提示词 |
...
| edit_page_url | string | 否 | 结果编辑页面地址 |
...
- 「先贤经典」灰度入口的可见用户范围、灰度标识文案和不可用状态下的替代提示是什么？
- 润色默认文风的具体选项名称、排序和默认选中项是否需要按文章类型动态变化？
- 自定义文风提示词是否需要长度限制、敏感词拦截或组织级禁用开关？
- 完整复刻排版模板的验收标准是什么，例如目录、编号、封面、页眉页脚和 PPT 母版允许的最大偏差范围？
- 结果编辑页面复用现有写作编辑器还是新建编辑器，是否需要保留逐字修订痕迹继续编辑？
